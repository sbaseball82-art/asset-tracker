...
サンプルの日付・EPS・売上はレイアウト検査用の作り物で、実際の決算日ではない
（画像にも SAMPLE と明記される）。本番の日付は毎回 Finnhub から取る。
...
見た目（配色・級数・余白・最大掲載社数・バッジ様式）は `config/theme.json`。
どちらもコードを触らずに編集できる。キャンバスも `canvas` を書き換えれば
追従する（既定は既存シリーズと同じ 1180×1450）。
...
- 発表タイミング：`bmo` → 寄付前 / `amc` → 引け後 / `dmh` → 場中 /
  不明 → **時間未定**（引け後と決めつけない）。
  バッジの様式は `theme.json` の `timing_styles` で決まる。引け後だけ金の塗りで
  目立たせ、寄付前・場中は枠線、時間未定は淡い塗り
- ロゴはカードの上に**直接**置く（白いパッドは敷かない）。ただし黒一色の
  透過ロゴは暗い背景で消えるため、不透明部の明るさが
  `logo.dark_luminance_threshold` を下回るときだけ白い角丸パッドを敷く
- ロゴが取れない会社は、ティッカー4文字を配色ブロックに載せて代替する
  （青 `#6BA8F5` / 青緑 `#7FD4C1` / 金 `#E0B45C` からティッカーのハッシュで決定）
- 罫線（ヘッダー下・日付見出しの右・フッター上）は金を薄く敷いたもの
- 掲載社数によって行の高さと余白は自動で変わる。12社の週は詰まり、
  数社の週は行を広げて余白で調整する
- `--sample` で描いた画像には **SAMPLE** バッジと
  「日付・数値はダミー」の注記が入る。ダミーの日付が本物の決算日として
  読まれないようにするため
