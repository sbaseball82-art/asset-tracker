...
2. **ISIN は必須**。協会CSVは ISIN が無いと引けない（協会コードだけでは
   `{"statusCode":null}` が返るだけ）ので、`None` を入れるとその銘柄は
   不安定な Yahoo! フォールバック一本になり、一時障害の日はそのまま
   `前日値(取得失敗)` に落ちる。実際に 89311265 が2日連続で落ちた。
   どうしても特定できないときだけ `None` を入れ、**その銘柄は取得が不安定になる**旨を
   必ずユーザーに報告する。勝手に ISIN を推測して埋めない。
3. 候補の ISIN が手に入ったら、`config.py` に書く前に協会CSVで引けるか検証する。
   別ファンドの ISIN でも CSV は返るので、**基準価額が一致するところまで**確認する。

   ```bash
   python3 scripts/diagnose_fund_source.py <協会コード> --isin <ISIN>
   ```

   ローカルから金融サイトに繋がらない環境では、Actions の
   fund-source-check ワークフロー（手動実行）で同じことができる。
4. `config.py` の `_FUND_META` に `"協会コード": ("表示名", "ISIN")` を追記する。
5. `holdings.json` の `fund` に協会コードと保有口数（整数）を追記する。
