...
## 手順0: 本当に失敗しているか確認する（手を動かす前に）

「Actionsが失敗したみたい」という依頼でも、**実際には成功していて遅延しただけ**のことがある
（GitHub Actionsのスケジュールは数分〜数時間遅れることがある公式仕様）。
また、**自分のローカルcheckoutは依頼時点より古いコミットで止まっていることがある**。
ローカルの data.json の日付が古い ≠ Actions失敗。

```bash
git fetch origin main && git log origin/main --oneline -3   # 今日の "update: YYYY-MM-DD" が既にあるか
```

GitHub MCPツールが使えるなら Actions の実行履歴（daily-asset-slide）の
status/conclusion も確認する。**今日分が既に成功していれば、手動実行は不要**。
origin/main の中身を検証（`git show origin/main:data.json` に対して
checklists.md 工程Aのチェックを当てる）して「成功済み・内容も正常」と報告して終了する。

本当に失敗している場合のみ、以下へ進む。

## 実行環境の判定（手動実行が必要になったら）
